...
printf("create table: constraint = '%s'\n", constraint);
...
printf("create table: constraint = '%s'\n", constraint);
         ...
         if (!strcmp(constraint, KEY)) {
            ...
            foreign += table_len + 1;
			//size_t column_len = strcspn(foreign, ")");
			//char *foreign_column = (char *)calloc(column_len + 1, sizeof(char));
			//strncpy(foreign_column, foreign, column_len);
            //column->foreign_key = create_column_by_name(trim(foreign_column));
...
int create_table_data_file(char *db_path, char *table_name) {
	char *path = (char *)malloc(strlen(db_path) * sizeof(char));
	strcpy(path, db_path);
